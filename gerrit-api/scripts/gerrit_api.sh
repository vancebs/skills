...
#
# Credentials are loaded from gerrit_config.json in the current working
# directory (highest priority), then fall back to environment variables.
#
...
if [[ -n "$_CONFIG_FILE" ]]; then
  ...
  _cfg_user=$(jq -r '.username // empty' "$_CONFIG_FILE" 2>/dev/null || true)
  _cfg_pass=$(jq -r '.password // empty' "$_CONFIG_FILE" 2>/dev/null || true)
  [[ -n "${_cfg_url}"  ]] && GERRIT_URL="$_cfg_url"
  [[ -n "${_cfg_user}" ]] && GERRIT_USERNAME="$_cfg_user"
  [[ -n "${_cfg_pass}" ]] && GERRIT_HTTP_PASSWORD="$_cfg_pass"
fi
...
cmd_help() {
  ...
  echo ""
  echo "Credentials loaded from gerrit_config.json (cwd) first, then env vars."
  echo ""
  echo "Commands:"
